...
class AboutRegularExpressions < EdgeCase::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    # When there are 0 occurrences of the pattern
  end
  ...
  #
  # Why?
  # They go all the way to the end of the pattern

  # ------------------------------------------------------------------

  def test_the_left_most_match_wins
    # NOTE: remember it matches the first one it finds, and asterisks means
    #       zero or more occurrences. Nice! (BAC)
  ...
  def test_slash_s_is_a_shortcut_for_a_whitespace_character_class
    # NOTE: tab and newline are whitespace! (BAC)
    assert_equal " \t\n", "space: \t\n"[/\s+/]
  end

  def test_slash_w_is_a_shortcut_for_a_word_character_class
    # NOTE:  This is more like how a programmer might define a word.
    # NOTE: First match is returned (BAC)
  ...
  def test_scan_is_like_find_all
    # NOTE: \w finds only the words (BAC)
    assert_equal ['one', 'two', 'three'], "one two-three".scan(/\w+/)
  end

  def test_sub_is_like_find_and_replace
    # NOTE: being tricky with substrings in the replace (BAC)
